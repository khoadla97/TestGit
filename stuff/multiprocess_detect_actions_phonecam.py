...
#url cua phone 
url = ('http://172.16.4.209:8080/shot.jpg')
#USE_WEBCAM = True
ACTION_FREQ = 8
...
DELAY = 125# ms, this limits the input around 16 fps. This makes sense as the action model was trained with similar fps videos.
#OBJ_GPU = "0"
#ACT_GPU = "2"
...
# frame reader
def read_frames(reader, frame_q, use_phonecam):
    if use_phonecam:
        time.sleep(10)
        frame_cnt = 0
        reader = cv2.VideoCapture('http://172.16.4.209:8080/video') #lay frame tu camera
        while True:
            #if frame_cnt % 5 == 0:    
...
def main():
    ...
    out_vid_path = out_vid_path if not use_phonecam else './output_videos/phonecam_output.mp4' 
    ...
    if use_phonecam:
        print("Using phonecam")
        reader = cv2.VideoCapture('http://172.16.4.209:8080/shot.jpg')
    ...
    frame_reader_p = Process(target=read_frames, args=(reader, frame_q, use_phonecam))
...
def visualize_detection_results(img_np, active_actors, prob_dict):
    ...
    for ii in range(len(active_actors)):
        ...
        #color = tuple(color) #chuyen ve tuple
        #print('sau khi tuple:',color)
        ...
        cv2.rectangle(disp_img, (left, top), (right,top+10*len(action_message_list)), (255,255,255), -1)
        ...
        for aa, action_message in enumerate(action_message_list):
            ...
            cv2.putText(disp_img, action_message, (left, top+5+offset), 0, 0.5, (0,255,0), 1)
